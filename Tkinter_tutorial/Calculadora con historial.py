...
## Ventana de texto que muestra el historial de operaciones

# El historial no funciona
def actualizar_ventana_texto(texto):
    ventana_texto.config(state = "normal") # Habilitar la edicion temporalmente
    ventana_texto.delete("1.0", END) # Borrar el contenido actual
    ventana_texto.insert(END, texto) # Insertar el nuevo texto
    ventana_texto.config(state = "disabled") # Deshabilitar la edicion manualmente


# Crear el widget text para la ventana de texto
ventana_texto = Text(root, padx = 10, pady = 34, state = "disabled", wrap = "word", borderwidth = 5)
ventana_texto.grid(row = 1, column = 3, rowspan= 6)
